...
use crate::{
    indexmap::IndexMap,
    tensor::{ Element, Kind, Q, TensorError, TensorResult },
};

use TensorError::*;

/// Represents a series of [`Element`]s as tensor-backed "slices" of a
/// ZX(H)-diagram.
///
...
impl Diagram {
    ...
    pub fn new(elems: Vec<Element>, scalar: Option<C64>) -> Self {
    ...
    pub fn contract(self) -> TensorResult<Element> {
        fn foldfn(mb_acc: Option<Element>, elem: Element)
            -> TensorResult<Option<Element>>
        {
        ...
        self.elems.into_iter()
            ...
            .map(|mb_res| {
                mb_res.unwrap_or_else(|| Element::new_scalar(1.0))
    ...
    /// Convert `self` to a [`ketbra::Diagram`][crate::ketbra2::Diagram]
    /// representation.
    ///
    /// Fails if any `Element` repeats a ket index without another `Element`'s
    /// matching bra index in between.
    pub fn as_ketbra(&self) -> TensorResult<crate::ketbra2::Diagram> {
        todo!()
    }
    ...
    /// matching bra index in between.
    pub fn as_graph(&self) -> TensorResult<crate::graph2::Diagram> {
            ...
            match elem.kind {
                ...
                Kind::Z(_) | Kind::X(_) | Kind::H(_) => {
                    ...
                    for out_wire in elem.outs() {
                        if wires.insert(out_wire, node_id).is_some() {
                            return Err(DuplicateIndex(Q::Ket(out_wire)));
                        }
                    }
                },
                Kind::Swap(a, b) => {
                ...
                Kind::Cup(a, b) => {
                    ...
                    if wires.insert(a, n).is_some() {
                        return Err(DuplicateIndex(Q::Ket(a)));
                    }
                    if wires.insert(b, n).is_some() {
                        return Err(DuplicateIndex(Q::Ket(b)));
                    }
                },
                Kind::Cap(a, b) => {
