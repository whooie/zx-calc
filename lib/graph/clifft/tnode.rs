use crate::graph::clifft::tphase::TPhase;
...
impl TNode {
    ...
    pub(crate) fn adjoint_mut(&mut self) {
        match self {
            Self::Z(ph) => { *self = TNode::Z(-*ph); },
            Self::Input => { *self = TNode::Output; },
            Self::Output => { *self = TNode::Input; },
        }
    }
    ...
    pub fn is_z2(&self, other: &Self) -> bool {
        matches!(
            (self, other),
            (Self::Z(_), Self::Z(_))
        )
    ...
    /// satisfy a predicate.
    pub fn is_z2_and<F>(&self, other: &Self, pred: F) -> bool
