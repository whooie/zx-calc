...
impl RuleFinder<ZX> for FuseMulti {
    type Output<'a> = FuseMultiData<'a, ZX>;
...
impl<'a> Rule<ZX> for FuseMultiData<'a, ZX> {
    fn simplify(self) {
...
impl RuleFinder<ZH> for FuseMulti {
    type Output<'a> = FuseMultiData<'a, ZH>;
...
impl<'a> Rule<ZH> for FuseMultiData<'a, ZH> {
    fn simplify(self) {
        let Self { dg, init: id0, others, ph_sum } = self;
